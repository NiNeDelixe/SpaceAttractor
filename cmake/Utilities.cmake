...
function(set_env_from_string env_string)
  ...
  foreach(env_var ${env_list})
    ...
    #           env_parts
    #           "${env_var}")
    string(FIND "${env_var}" "=" pos)
    if(NOT pos EQUAL -1)
      string(SUBSTRING "${env_var}" 0 ${pos} env_name)

      math(EXPR value_pos "${pos} + 1")
      string(SUBSTRING "${env_var}" ${value_pos} -1 env_value)
    endif()
    ...
    if("${env_parts_length}" EQUAL "2")
      ...
      #message(STATUS "set env_name: ${env_value} to env_value: " ENV{${env_name}})
      ...
      if("${env_name}" EQUAL "PATH")
        ...
        foreach(p ${path_list})
          list(APPEND CMAKE_PROGRAM_PATH "${p}")
        endforeach()
      endif()
    endif()
  endforeach()
